...
func StartGame() {
	...
	element := document.Call("getElementById", "game")
	element.Set("class", "row")
}
...
func startOnlineGame(ctx context.Context) {
	...
	startMsg, _ := json.Marshal(map[string]any{
		"type": "start",
		"payload": game.StartGameMessage{
			Duration: 10,
		},
	})
	ws.Write(ctx, websocket.MessageText, startMsg)
	...
	resp := game.StartGameWSMsg{}
	if err := json.Unmarshal(msg, &resp); err != nil {
		fmt.Println("json unmarshal: ", err)
		return
	}

	fmt.Println(resp.Payload.Tile)
	engine := chess.NewEngine()
	if resp.Payload.Tile == "black" {
		engine.SwitchTurn()
	}
}
...
func (ui *ChessUI) handlePickupPiece(x, y int) {
	if ui.game.GetBoard()[y][x] != nil {
		...
		ui.pickedPiece = ui.game.GetBoard()[y][x]
	}
}
